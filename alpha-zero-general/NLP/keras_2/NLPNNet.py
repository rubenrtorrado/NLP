...
class NLPNNet():
    def __init__(self, game, args):
        ...
        extract1 = LSTM(10, return_sequences=True)(self.input_boards)
        drop1 = Dropout(0.2)(extract1)
        extract2 = LSTM(10, return_sequences=True)(drop1)
        drop2=Dropout(0.2)(extract2)
        #extract3=LSTM(700, return_sequences=True)(drop2)
        #drop3 = Dropout(0.2)(extract3)
        self.pi=Dense(self.action_size, activation='softmax', name='pi')(drop2)
        self.v=Dense(1, activation='tanh',name='v')(drop2)
        self.model = Model(inputs=self.input_boards, outputs=[self.pi, self.v])
        self.model.compile(loss=['categorical_crossentropy', 'mean_squared_error'], optimizer=Adam(args.lr))
