...
extract1 = LSTM(700, return_sequences=True)(visible)
extract2 = LSTM(700, return_sequences=True)(extract1)
drop1 = Dropout(0.2)(extract2)
extract3 = LSTM(700, return_sequences=True)(drop1)
drop2 = Dropout(0.2)(extract3)
pi = Dense(37, activation='softmax', name='pi')(drop2)
v = Dense(1, activation='tanh', name='v')(drop2)
model = Model(inputs=visible, outputs=[pi, v])
model.compile(loss=['categorical_crossentropy', 'mean_squared_error'], optimizer=Adam(0.01))
print(model.summary())