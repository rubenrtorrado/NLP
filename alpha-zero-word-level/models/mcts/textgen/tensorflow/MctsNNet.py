...
class MctsNNet(object):

    def __init__(self, lstm, sess, update_rate=.8):
        self.lstm = lstm
        ...
        #     cond=lambda i, _1, _2, _3, _4: i < given_num,
        ...
        # Probabilities for the next words
        h_t = self.g_recurrent_unit(x_t, h_tm1)  # hidden_memory_tuple
        o_t = self.g_output_unit(h_t)  # batch x vocab , logits not prob
        ...
        fc1 = Dropout(Relu(Dense(o_t, 32)), rate=self.dropout)  # batch_size x 1024
        #s_fc2 = Dropout(Relu(Dense(s_fc1, 32), axis=1, training=self.isTraining), rate=self.dropout)  # batch_size x 512
        # self.pi = Dense(s_fc2, self.action_size)  # batch_size x self.action_size
        # self.prob = tf.nn.softmax(self.pi)
        self.v = Tanh(Dense(fc1, 1))  # batch_size x 1

        # TODO: Loss function for training



    def predict(self, board):

        # FIXME: Change network to accept a batch_size=1
        input_x = np.full((self.batch_size, self.sequence_length), fill_value=self.num_vocabulary-1)
        input_x[0, 0:len(board.tokens)] = board.tokens
        # input_x = np.array(board.tokens, np.int32)
        # input_x = input_x[np.newaxis, : ]
        feed = {self.x: input_x, self.given_num: len(board.tokens), self.dropout: 0 }
        prob, v = self.sess.run([self.prob, self.v], feed)
        return prob[0], v[0]
