...
class MctsNNet():
    def __init__(self, game, args):
        ...
        self.input_boards = Input(shape=self.board_dim)
        embedding = Embedding(input_dim=game.num_vocabulary,
                              output_dim=32,
                              input_length=game.sequence_length)(self.input_boards)
        # Single layer LSTM
        # x = LSTM(32, dropout=0.2, return_sequences=False)(embedding)

        # Mulitple-layers
        x = LSTM(32, dropout=0.2, return_sequences=True)(embedding)
        x = LSTM(32, dropout=0.2, return_sequences=True)(x)
        x = LSTM(32, dropout=0.2, return_sequences=False)(x)

        # alpha-zero policy network output
        ...
        # pre-training
        # self.lm = Model(inputs=self.input_boards, outputs=[self.pi])
        # self.lm.compile(loss=['categorical_crossentropy'], optimizer=Adam(args.lr))
