...
class DataLoader():
    ...
    def next_example(self, return_eof=False):
        if self.example_pointer >= len(self.sequence_batch[self.pointer]):
            self.pointer = (self.pointer + 1) % self.num_batch
        ...
        ret = self.sequence_batch[self.pointer][self.example_pointer]
